...
func getDataForList(c *gophercloud.ServiceClient,from int64, to int64, points int32) string {
...
func getURLForPoints(c *gophercloud.ServiceClient,metric string, from int64, to int64, points int32) string {
	fmt.Println(c.ServiceURL(root,metric+"?from="+fmt.Sprintf("%d",from)+"&to="+fmt.Sprintf("%d",to)+"&points="+fmt.Sprintf("%d",points)))
...
func getURLForPointsWithSelect(c *gophercloud.ServiceClient,metric string, from int64, to int64, points int32, sel string) string {
	return c.ServiceURL(root,metric+"?from="+fmt.Sprintf("%d",from)+"&to="+fmt.Sprintf("%d",to)+"&points="+fmt.Sprintf("%d",points)+"&select="+sel)
}
...
func getURLForResolutionWithSelect(c *gophercloud.ServiceClient,metric string, from int64, to int64, resolution string, sel string) string {
	return c.ServiceURL(root,metric+"?from="+fmt.Sprintf("%d",from)+"&to="+fmt.Sprintf("%d",to)+"&resolution="+resolution+"&select="+sel)
}
